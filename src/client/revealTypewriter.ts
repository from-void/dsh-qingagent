...
export const DEFAULT_REVEAL_TIMEOUT_GRACE_MS = 3_000

/** 动画定时器失活时的总预算；到点后消费侧必须清掉 reveal 请求。 */
export function revealHardTimeoutMs(frameCount: number): number {
  return Math.max(0, Math.floor(frameCount)) * DEFAULT_REVEAL_STEP_DELAY_MS
    + DEFAULT_REVEAL_TAIL_HOLD_MS
    + DEFAULT_REVEAL_TIMEOUT_GRACE_MS
}
