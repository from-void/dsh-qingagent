...
const QINGML_NESTED_BULLET_LIST_EXAMPLE = '<ul><li>大类甲<ul><li>具体项一</li><li>具体项二</li></ul></li><li>大类乙</li></ul>'
const QINGML_NESTED_TASK_LIST_EXAMPLE = '<tasks><task>父任务<tasks><task>子任务</task></tasks></task></tasks>'
const QINGML_FOOTNOTE_EXAMPLE = '<p>这项结论已有研究支持<footnote id="source_1">《资料甲》，第 12 页。</footnote>。</p>'
const QINGML_INLINE_MATH_EXAMPLE = '<p>质能关系为 <math>E=mc^2</math>。</p>'
const QINGML_BLOCK_MATH_EXAMPLE = '<math-block>\\int_0^1 x^2\\,dx=\\frac{1}{3}</math-block>'

describe('QingML 流边界与摘要', () => {
  it('要求元数据标题与正文纸面大标题双写且文字一致', () => {
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('文档标题写进最前的 <title>')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('同时在正文开头写一个文字完全一致的 <h1> 作为纸面大标题')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('其余 h2-h6 表示章节层级')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('完整 QingML 全文一次放入 qing_write_draft 的 qingml 参数')
  })
  ...
  it('嵌套列表正面样例本身是完整 QingML 顶层块', () => {
    for (const example of [QINGML_NESTED_BULLET_LIST_EXAMPLE, QINGML_NESTED_TASK_LIST_EXAMPLE]) {
      ...
      expect(QINGAGENT_SYSTEM_PROMPT).toContain(example)
    }
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('列表层级靠嵌套表达，不靠标题')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('必须用嵌套列表，不用「小标题+平级列表」')
  })
  ...
  it('系统提示硬性禁止 Markdown/GFM 脚注和公式源语法', () => {
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('【源语法纪律】')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('严禁写 [^x]、[^x]: …')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('严禁写 $…$、$$…$$')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('脚注用 <footnote id="x">注文</footnote>')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('行内公式用 <math>…</math>')
    expect(QINGAGENT_SYSTEM_PROMPT).toContain('块级公式用 <math-block>…</math-block>')
  })
