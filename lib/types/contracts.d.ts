import type { DocSuggestion, ExternalAssetUploadJsonRequest as QingExternalAssetUploadJsonRequest, ExternalAssetUploadResponse as QingExternalAssetUploadResponse, ExternalDocReplaceRequest as QingExternalDocReplaceRequest, ExternalDocReplaceResponse as QingExternalDocReplaceResponse, ExternalPmDocReadResponse as QingExternalPmDocReadResponse, ExternalReviewRenderModelResponse as QingExternalReviewRenderModelResponse, PmDoc } from '@qingagent/contract-ts';
export type EngineState = 'online' | 'offline' | 'starting' | 'handshake-failed';
export type EngineStatusReason = 'instance-missing' | 'instance-invalid' | 'instance-process-exited' | 'connection-refused' | 'connection-timeout' | 'unauthorized' | 'health-http-error' | 'health-response-invalid' | 'version-mismatch' | 'protocol-incompatible';
...
type OptionalCharCount<T> = T extends {
    charCount: number;
} ? Omit<T, 'charCount'> & {
    charCount?: number;
} : T;
...
export declare const DRAFT_MARK_COLORS: readonly ["ink", "gray", "slate", "brown", "red", "orange", "amber", "yellow", "lime", "green", "sage", "mint", "teal", "cyan", "sky", "blue", "indigo", "violet", "purple", "magenta", "pink", "rose", "sand", "lavender"];
